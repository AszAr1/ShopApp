...
import { useHoodiesProps } from "../models/productsProps";
...
export const useHoodies = create<useHoodiesProps>((set) => ({
    ...
    async getLimitedHoodies(limit) {
        try {
            HooidesService.getLimitedHoodies(limit).then((response) =>
    ...
    async getHoodies() {
        try {
            HooidesService.getHooides().then((response) =>
                set({ hoodies: response.data }),
            );
        } catch (e) {
    ...
    async getOneHoodie(productId) {
        try {
            HooidesService.getOneHoodie(productId).then((response) =>
